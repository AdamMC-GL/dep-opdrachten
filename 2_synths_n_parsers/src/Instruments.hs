...
-- | Function silence generates a silence for 'sec' duration, a floatSound containing 0.0 (`sampleRate * seconden`) times
-- takes one argument, the duration
silence :: Seconds -> Sound
silence sec = floatSound (replicate(round(sampleRate * sec)) 0.0)
...
-- | defaultSquare is a squareWave instrument combined with an attack and release modifier
...
-- | defaultTriange is a triangleWave instrument combined with an attack and release modifier
...
-- | Function pad adds a quarter-note in front and behind a list of notes
-- takes one argument, a list of notes
pad :: [Note] -> [Note]
pad note = Pause Quarter : note ++ [Pause Quarter]
